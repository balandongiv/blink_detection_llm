...
onsets and offsets are estimated. If the analysis is run in the fallback mode
described above, \(\mathcal{S}\) is the full valid epoch set; otherwise it is
the set of epochs identified as suspicious in Stage A.
...
Finally, the accepted intervals are mapped from concatenated coordinates back
to epoch-local coordinates using the known epoch boundaries. If an accepted
...
onset index or time, and an offset or duration. The extraction remains
