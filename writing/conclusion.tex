...
stable across this range. Strategy comparisons showed that the proposed epoch-aware
pipeline substantially improves over naive concatenation and a community baseline (MNE
annotate\_amplitude), while a direct Bayesian-optimisation ablation can remain competitive
in macro-F1. Finally, the IoU sweep highlights that evaluation strictness must be reported
explicitly: stricter overlap thresholds lead to markedly lower F1 even when detections are
near-misses.
