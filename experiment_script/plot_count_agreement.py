"""R2 - Predicted-versus-ground-truth blink-count agreement.
...
runs_second_iteration/exp2_*/. Cross-checked against exp8 (Proposed-Med
n_predicted vs n_gt_total, Raja r=0.886). Run inside conda env double_threshold_algo.
"""
...
for c in CONDS:
    parts = []
    for ds in ["raja", "cao"]:
        b = best_per_session(load(ds)[load(ds).condition == c])
        b["pred"] = b.det_tp + b.det_fp
        b["truth"] = b.det_tp + b.det_fn
...
fig.tight_layout()
fig.savefig(FIGDIR / "fig_count_agreement.pdf", bbox_inches="tight")
fig.savefig(FIGDIR / "fig_count_agreement.png", dpi=150, bbox_inches="tight")
print("wrote fig_count_agreement.pdf/.png")

# ---------- table ----------
SRC = "% Source: runs_second_iteration/exp2_*; script experiment_script/plot_count_agreement.py"
L = [SRC, r"\begin{table}[ht]", r"  \centering",
     r"  \caption{Agreement between the predicted blink count (TP\,+\,FP) and the true count "
     r"(TP\,+\,FN) per session, pooled over Raja and Cao2018 ($n=104$) at the best-channel-per-session "
...
(ER / "tab_count_agreement.tex").write_text("\n".join(L) + "\n", encoding="utf-8")
print("wrote tab_count_agreement.tex")
