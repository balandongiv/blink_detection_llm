"""Run Exp 2 (Raja) — full strategy comparison — no argparse needed.
...
Output CSVs go to runs/exp2_raja/.

How to change settings
-----------------------
  GROUPS_TO_RUN     — best channels from Exp 1 (Raja); update if Exp 1 changes
  N_JOBS            — None → all cores minus 1; 1 → serial (debug)
  HEARTBEAT_EVERY_S — Telegram heartbeat interval in seconds (default 900 = 15 min)

Resume support
--------------
Each session has one per-session CSV file (contains rows for all conditions + groups).
...
--------------
  ...
  selection         — channel group name (e.g. "frontal", "single:E22")
...
def _run_one_group_all_conditions(
    ...
    Returns (rows, errors).  Each row is one (condition, group) metric record.
    """
    rows: list[dict] = []
    errs: list[str] = []
...
def main() -> None:
    ...
    # Group summary rows by selection
    sel_order = RAJA_SELECTION_ORDER
    sels_done: set = set()
