...
Run from REPO ROOT:
    conda run -n double_threshold_algo python experiment_script/run_std30_orchestrator.py

What it does
------------
1. Smoke-tests the pyblinker install.
2. For each experiment (exp1→exp5, skip exp6, exp7→exp8):
   a. Creates a temporary patched copy of the run script pointing to *_std30.yaml
      and writing to runs/*_std30/ (original baseline untouched).
   b. Runs the patched script as a subprocess, waits for completion.
   c. Reads the new *_results.csv and baseline *_results.csv.
   d. Computes macro-F1 and micro-F1 for Proposed-Med rows on both datasets.
   e. Calls codex CLI (--dangerously-bypass-approvals-and-sandbox) for algorithm-performance insight.
   f. Sends the comparison table + AI insight to Telegram.
   g. Gates on pass/fail: stops immediately if either F1 regresses.
...
EXPERIMENTS = [
        ...
        "runs": [
            {
                ...
                "std30_yaml": "exp1_channel_selection_raja_std30.yaml",
            ...
            },
            {
                ...
                "std30_yaml": "exp1_channel_selection_cao2018_std30.yaml",
    ...
    {
        "exp_name": "exp2",
        "description": "Strategy comparison (BLINKER / MNE / Proposed-Mean / Proposed-Med)",
        "runs": [
            {
                "dataset": "raja",
                "script": "experiment_script/run_exp2_raja.py",
                "orig_out": "runs/exp2_raja",
                "std30_out": "runs_second_iteration/exp2_raja",
                "orig_yaml": "exp2_strategy_comparison.yaml",
                "std30_yaml": "exp2_strategy_comparison_std30.yaml",
                "results_csv": "exp2_strategy_comparison_raja_results.csv",
            },
            {
                "dataset": "cao2018",
                "script": "experiment_script/run_exp2_cao2018.py",
    ...
    },
    {
        ...
        "runs": [
            {
                "dataset": "raja",
                "script": "experiment_script/run_exp3_raja.py",
                "orig_out": "runs/exp3_raja",
                "std30_out": "runs_second_iteration/exp3_raja",
                "orig_yaml": "exp3_epoch_duration.yaml",
                "std30_yaml": "exp3_epoch_duration_std30.yaml",
                "results_csv": "exp3_epoch_duration_raja_results.csv",
            },
            {
                "dataset": "cao2018",
                "script": "experiment_script/run_exp3_cao2018.py",
    ...
    },
    {
        ...
        "runs": [
            {
                "dataset": "raja",
                "script": "experiment_script/run_exp4_raja.py",
                "orig_out": "runs/exp4_raja",
                "std30_out": "runs_second_iteration/exp4_raja",
                "orig_yaml": "exp4_boundary_tolerance.yaml",
                "std30_yaml": "exp4_boundary_tolerance_std30.yaml",
                "results_csv": "exp4_boundary_tolerance_raja_results.csv",
            },
            {
                "dataset": "cao2018",
                "script": "experiment_script/run_exp4_cao2018.py",
    ...
    },
    {
        ...
        "runs": [
            {
                "dataset": "raja",
                "script": "experiment_script/run_exp5_raja.py",
                "orig_out": "runs/exp5_raja",
                "std30_out": "runs_second_iteration/exp5_raja",
                "orig_yaml": "exp5_nmin_sensitivity.yaml",
                "std30_yaml": "exp5_nmin_sensitivity_std30.yaml",
                "results_csv": "exp5_nmin_sensitivity_raja_results.csv",
            },
            {
                "dataset": "cao2018",
                "script": "experiment_script/run_exp5_cao2018.py",
    ...
    # exp6 is the ablation that generated the 3.0 decision — skip re-run.
    {
        "exp_name": "exp7",
        ...
        "runs": [
            {
                ...
                "orig_yaml": "exp7_epoch_health_effect.yaml",
                "std30_yaml": "exp7_epoch_health_effect_std30.yaml",
    ...
    },
    {
        ...
        "runs": [
            {
                "dataset": "raja",
                "script": "experiment_script/run_exp8_raja.py",
                "orig_out": "runs/exp8_raja",
                "std30_out": "runs_second_iteration/exp8_raja",
                "orig_yaml": "exp8_long_blink_analysis.yaml",
                "std30_yaml": "exp8_long_blink_analysis_std30.yaml",
                "results_csv": "exp8_long_blink_raja_results.csv",
            },
            {
                "dataset": "cao2018",
                "script": "experiment_script/run_exp8_cao2018.py",
...
def _patch_script(src_path: Path, run_cfg: dict) -> Path:
    """Return path to a patched temp script; caller must delete it."""
    text = src_path.read_text(encoding="utf-8")

    # Patch OUT_DIR
    text = text.replace(
        f'OUT_DIR = Path("{run_cfg["orig_out"]}")',
        f'OUT_DIR = Path("{run_cfg["std30_out"]}")',
    )
    # Patch YAML filename (appears in the load_exp_config() call)
    text = text.replace(
        f'"{run_cfg["orig_yaml"]}"',
        f'"{run_cfg["std30_yaml"]}"',
    )
...
def _run_script(tmp_script: Path) -> int:
    """Run the patched script in the current Python env and return exit code."""
...
def _load_csv(path: Path) -> list[dict]:
    ...
    numeric = {"det_tp", "det_fp", "det_fn", "det_precision", "det_recall", "det_f1"}
    coerced = []
    for r in rows:
        row = dict(r)
        for k in numeric:
...
def _compute_metrics(rows: list[dict]) -> dict | None:
    ...
    f1_vals = [v for r in rows for v in [_f(r, "det_f1")] if v is not None]
    p_vals  = [v for r in rows for v in [_f(r, "det_precision")] if v is not None]
    rc_vals = [v for r in rows for v in [_f(r, "det_recall")] if v is not None]
    ...
    tp = sum(_f(r, "det_tp") or 0.0 for r in rows)
    fp = sum(_f(r, "det_fp") or 0.0 for r in rows)
    fn = sum(_f(r, "det_fn") or 0.0 for r in rows)
...
def main() -> None:
    ...
    for exp_cfg in EXPERIMENTS:
        ...
        for run_cfg in runs:
            ...
            try:
                tmp_path = _patch_script(src_path, run_cfg)
                set_state(last_step=f"{exp_name} {dataset} patched", next_step=f"run {exp_name} {dataset}")
                ...
                out_sessions_dir = REPO_ROOT / run_cfg["std30_out"]
                heartbeat.set_phase(exp_name, dataset, out_sessions_dir, expected_sessions)

                rc = _run_script(tmp_path)
