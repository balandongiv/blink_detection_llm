"""Run Exp 7 (Cao2018) — epoch-health filter effect — no argparse needed.

Just press the Play button in IntelliJ IDEA.

What this experiment tests
--------------------------
Compares use_epoch_health=False (baseline) vs use_epoch_health=True (epoch filtering)
using the best channel groups from Exp 1 (Cao2018).

Configuration is read from experiment_script/setup/exp7_epoch_health_effect.yaml.
Output CSVs go to runs/exp7_cao/.

Resume support
--------------
runs/exp7_cao/sessions/<session>__health<0_or_1>.csv
"""
...
def _heartbeat_thread(stop_event, progress, n_total, start_time):
    while not stop_event.wait(HEARTBEAT_EVERY_S):
        ...
        _send_telegram(
            f"[Exp7 Cao2018] Heartbeat  {done}/{n_total}  "
...
def main():
    ...
    _tg7c = per_group_telegram_message(
        tag="[Exp7 Cao2018] COMPLETE",
