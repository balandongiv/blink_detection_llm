...
The YAML is a HARD-APPROVAL GATE: the default group is ``"all_channel"`` (no
change), and selecting any other group requires ``approved_by`` to be set in
the YAML — otherwise :func:`load_group_for_dataset` raises, so an unapproved
group can never silently take effect. ``exp1_channel_selection_*`` deliberately
does NOT call this (it must see the full montage to compare every group).
"""
...
def load_group_for_dataset(dataset: str, *, yaml_path: Path = CHANNEL_GROUP_YAML) -> str:
    """Return the approved group for *dataset* (default ``"all_channel"``).

    Raises ``ValueError`` if a non-``"all_channel"`` group is selected without
    ``approved_by`` being filled in the YAML (the hard-approval gate).
    """
    cfg = _load_config(str(yaml_path))
    group = str((cfg.get("stage_a_channel_group") or {}).get(dataset, "all_channel"))
    if group != "all_channel" and not cfg.get("approved_by"):
        raise ValueError(
            f"{yaml_path.name} selects Stage-A group '{group}' for dataset "
            f"'{dataset}', but 'approved_by' is empty. A non-'all_channel' group "
            f"requires explicit human/agent approval — set 'approved_by' and "
            f"'approved_date'."
        )
    return group
...
def apply_stage_a_channel_group(
    ...
    *,
    ...
    Returns *prepared* unchanged when the group is ``"all_channel"`` or the
    dataset has no region config (e.g. murat2018), so the default is a true no-op.
    """
    group = load_group_for_dataset(dataset, yaml_path=yaml_path)
    region_yaml = _REGION_YAML_BY_DATASET.get(dataset)
    if group == "all_channel" or region_yaml is None:
        if dataset not in _WARNED_ALL_CHANNELS:
            _WARNED_ALL_CHANNELS.add(dataset)
            logger.warning(
                "using ALL channels for '%s' — no channel group is selected in "
                "%s (group='all_channel'). Run exp1 (channel selection), then record the "
                "winning group and 'approved_by' in %s to restrict Stage A to the chosen "
                "montage.",
    ...
    if group not in groups:
        raise ValueError(
            f"group '{group}' not available for {dataset}. "
            f"Available: {sorted(groups)}"
        )
    keep = set(groups[group])
    idx = np.array([i for i, ch in enumerate(channel_names) if ch in keep], dtype=int)
    if idx.size == 0:
        raise ValueError(f"group '{group}' resolved to 0 channels for {dataset}.")
    logger.info("channel group '%s' (%s) → %d/%d channels",
