"""Run Exp 3 (Raja) — epoch-duration sensitivity — no argparse needed.
...
A robust detector should show similar det_f1 across durations, because the
underlying physiology of blinks does not change with the epoch length.

Configuration is read from experiment_script/setup/exp3_epoch_duration.yaml.
Output CSVs go to runs/exp3_raja/.

How to change settings
-----------------------
  EPOCH_DURATIONS       — list of durations to sweep (seconds)
  REFERENCE_EPOCH_DUR_S — reference duration for relative comparison (30 s)
...
Each (session, epoch_duration) combination has its own CSV:
  runs/exp3_raja/sessions/<session>__<N>s.csv

Set OVERWRITE = False to skip already-computed combinations.
...
# Reference duration for result commentary (matched to YAML).
...
def _process_one_task(task: tuple[dict, float]) -> tuple[str, float, list[dict], list[str]]:
    ...
    Returns (session_name, epoch_duration_s, metric_rows, error_messages).
    """
...
def main() -> None:
    ...
    # Telegram: per-channel, per-duration breakdown (median only).
    # Show combined groups (with each member channel) and single channels; flag inversions.
    ...
    # Inversions: at reference duration, does any individual channel beat frontal's BEST channel?
    ref_frontal_rows = [r for r in median_rows
                        if r["selection"] == "frontal"
                        and float(r["epoch_duration_s"]) == ref_dur]
    ref_frontal_best = max((float(r["det_f1"]) for r in ref_frontal_rows), default=None)
    inv_lines = []
    if ref_frontal_best is not None:
        for sel in single_sels:
            for ch_row in [r for r in median_rows
                           if r["selection"] == sel
                           and float(r["epoch_duration_s"]) == ref_dur]:
                if float(ch_row["det_f1"]) >= ref_frontal_best:
                    inv_lines.append(
                        f"  {sel}/{ch_row['channel_in_group']} "
                        f"F1={float(ch_row['det_f1']):.4f} >= frontal_best {ref_frontal_best:.4f}"
                    )

    parts = [
        f"[Exp3 Raja] COMPLETE",
        f"Sessions: {len(pairs)}  Tasks: {len(pairs)*len(EPOCH_DURATIONS)}"
        f"  Errors: {len(errors)}  Elapsed: {elapsed_min:.1f} min",
        f"Reference epoch: {int(ref_dur)}s  (* = reference dur)",
        "",
        _tg_block(combined_sels, "=== Combined channel groups (per channel) ==="),
        "",
        _tg_block(single_sels, "=== Single channels ==="),
    ]
    if inv_lines:
        parts += ["", "*** INVERSIONS: single ch F1 >= best frontal channel at ref dur ***"] + inv_lines
    else:
