"""Run Exp 5 (Cao2018) — min-flagged-epochs sensitivity — no argparse needed.

Just press the Play button in IntelliJ IDEA.

What this experiment tests
--------------------------
Sweeps min_flagged_epochs across {1, 2, 3, 5} using the best channels from Exp 1
(Cao2018) and median centre.  Tests whether the threshold estimation is stable
when the minimum required number of flagged epochs changes.

Configuration is read from experiment_script/setup/exp5_nmin_sensitivity.yaml.
Output CSVs go to runs/exp5_cao/.

Resume support
--------------
Each (session, nmin) combination has its own CSV:
  runs/exp5_cao/sessions/<session>__nmin<N>.csv
"""
...
def _session_csv(out_dir, session_name, nmin):
...
def _process_one_task(task):
    ...
    kw = dict(
        ...
        use_epoch_health=False, verbose=False, min_flagged_epochs=nmin,
    )
    groups = selection_group_names(
        pair, region_yaml=CAO_REGION_YAML,
...
def _heartbeat_thread(stop_event, progress, n_total, start_time):
    while not stop_event.wait(HEARTBEAT_EVERY_S):
        ...
        _send_telegram(
            f"[Exp5 Cao2018] Heartbeat  {done}/{n_total}  "
...
def condition_summary_rows_nmin(records, dataset_label):
    ...
    for (nmin, sel, ch, center), bucket in buckets.items():
        ...
        out.append({
            "dataset": dataset_label, "min_flagged_epochs": nmin, "selection": sel,
...
def main():
    ...
    for pair in pairs:
        for nmin in NMIN_VALUES:
            cp = _session_csv(out_dir, pair["name"], nmin)
    ...
    def _store(name, nmin, rows, errs):
        ...
        progress["done"] += 1; progress["latest"] = f"{name} nmin={nmin}"
            ...
            with ProcessPoolExecutor(max_workers=n_jobs) as ex:
                ...
                for fut in as_completed(fut_map):
                    try:
                        ...
                        errors.append(f"ERROR: {exc}"); progress["done"] += 1
    ...
    numeric_keys = {
        "min_flagged_epochs", "pct_flagged", "n_flagged", "n_blink_epochs",
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        "experiment": f"exp5_nmin_sensitivity_{DATASET}", "nmin_values": NMIN_VALUES,
    ...
    _tg5c = per_group_telegram_message(
        tag="[Exp5 Cao2018] COMPLETE",
