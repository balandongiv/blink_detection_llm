...
Run sequence:
  ...
  2. exp1_step_b_get_best_region_channel.py runs after those summary CSVs exist.
     ...
     dataset:
       ...
       - Top 4 regional groups       (selection NOT "single:" and NOT "all_channel")

Selection criterion: highest det_f1_macro for the Proposed algorithm
  (center_method="median"). f1_macro is the mean of each session's own F1
  (every session/subject weighted equally, regardless of how many blinks it
  has) rather than f1_micro (pooled tp/fp/fn, which lets high-blink-count
  sessions dominate) — see exp1_channel_selection_*_summary.csv, which
  reports both. The full-montage "all_channel" selection is a reference, not
  a region, so it is excluded from the regional ranking.
...
EXCLUDE_GROUPS = {"all_channel"}  # the full montage is a reference, not a region group
...
def select_top_channels(
    ...
    *,
    ...
    """Return top individual and regional channel dicts (highest det_f1_macro)."""
    filtered = [r for r in rows if r.get("center_method") == center_method]

    individual = sorted(
        [r for r in filtered if r["selection"].startswith("single:")],
        key=lambda r: _safe_float(r["f1_macro"]), reverse=True,
    )
    regional = sorted(
        [r for r in filtered
    ...
    def _to_record(r: dict) -> dict:
        return {
            ...
            "det_precision": round(_safe_float(r["precision_macro"]), 4),
            "det_recall": round(_safe_float(r["recall_macro"]), 4),
            "det_f1": round(_safe_float(r["f1_macro"]), 4),
...
def main() -> None:
    ...
    yaml_lines = [
        ...
        "# Criterion: highest det_f1_macro for proposed_median (center=median).",
        "# Top-4 region groups + top-4 single channels per dataset; 'all_channel' excluded.",
        "# run_exp2..exp8 should use <dataset>.groups_to_run as their GROUPS_TO_RUN set.",
        f"source_runs: {runs_name}",
        "criterion: proposed_median det_f1_macro (center=median)",
    ]
    ...
    md = [
        ...
        "Criterion: highest `det_f1_macro` using `proposed_median` (center=median).  ",
