"""Run Exp 6 (Raja) — std_threshold sensitivity — no argparse needed.

Just press the Play button in IntelliJ IDEA.

What this experiment tests
--------------------------
The ``std_threshold`` parameter is the multiplier k applied to the MAD (median
absolute deviation) when computing the detection threshold at Stage B:

    threshold = median + k * MAD      (Proposed-Med)
    threshold = mean   + k * std      (Proposed-Mean)

This experiment sweeps std_threshold across {2.5, 3.0, 3.5, 4.0} using the best
channel groups from Exp 1 (Raja) and median centre.

Configuration is read from experiment_script/setup/exp6_morphological.yaml.
Output CSVs go to runs/exp6_raja/.

How to change settings
-----------------------
  STD_THRESHOLDS    — multiplier values to sweep
...
Each (session, std_threshold) combination has its own CSV:
  runs/exp6_raja/sessions/<session>__std<value>.csv

Output columns (beyond Exp1 baseline)
--------------------------------------
  std_threshold_used  — std_threshold value for this row (to distinguish from
                        the column name collision with yaml config value)
"""
...
OUT_DIR = Path("runs/exp6_raja")
OVERWRITE = False
MAX_SESSIONS = None
N_JOBS = 16  # 16 of 24 logical threads (i7-13700F)
HEARTBEAT_EVERY_S = 900

# std_threshold multiplier values to sweep.
...
def _process_one_task(task):
    ...
    groups = selection_group_names(
        pair, region_yaml=RAJA_REGION_YAML,
...
def _heartbeat_thread(stop_event, progress, n_total, start_time):
    while not stop_event.wait(HEARTBEAT_EVERY_S):
        ...
        _send_telegram(
            f"[Exp6 Raja] Heartbeat  {done}/{n_total}  "
...
def main():
    ...
    _tg6r = per_group_telegram_message(
        tag="[Exp6 Raja] COMPLETE",
