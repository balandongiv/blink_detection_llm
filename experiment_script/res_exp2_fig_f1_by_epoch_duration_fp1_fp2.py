...
Data extraction (for auditing):
  Source CSV : publication_results/exp2_{raja,cao2018}/exp3_epoch_duration_{raja,cao2018}_
               results.csv, via ``paper_data.load("exp3", ds)``. (The "exp3" argument is
               ``paper_data``'s internal semantic key for epoch-duration content; since the
               2026-09-14 folder rename, it now resolves to the ``publication_results/
               exp2_*`` directories that match the manuscript's own Experiment 2 numbering
               -- see ``experiment_script/setup/exp_path.yaml``'s note.)
  Filter     : ``center_method == "median"`` by default (``--center-method`` CLI flag,
               Proposed-Med; pass ``mean`` for Proposed-Mean) AND ``selection ==
               "all_channel"`` (full 32-channel montage gate).
  Channels   : Fp1 and Fp2 only, reported as two separate series -- not a region/montage
               aggregate. Chosen because Experiment 1 found them the two most frequently
               best-channel-per-session electrodes on both corpora.
...
def build_figure(block_means: dict, block_p: dict) -> None:
    ...
    ``res_exp1_fig_single_channel_vs_montage_significance.py``): a plain, unrotated, bold asterisk, with the
