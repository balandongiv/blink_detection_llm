"""Experiment 3 (Raja): epoch-duration sensitivity, best-channel groups from exp1.

Replaces the old exp3_epoch_duration.py / run_exp3_raja.py. For every epoch
...
Output CSVs go to runs/exp3_raja/ by default.
...
Full sweep::
    ...
    python experiment_script/exp3_a_epoch_duration_raja.py --out-dir runs/exp3_raja

Quick smoke test::

    python experiment_script/exp3_a_epoch_duration_raja.py --max-sessions 1 --n-jobs 1
"""
...
DEFAULT_OUT_DIR       = REPO_ROOT / Path(_PATH_CFG["out_dirs"]["exp3"]["raja"])
...
DEFAULT_GROUPS = ",".join(_EXP_CFG["groups"]["raja"])
