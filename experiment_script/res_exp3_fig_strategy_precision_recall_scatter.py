...
Data extraction (for auditing):
  Source CSV : resolved at import time from ``experiment_script/setup/exp_path.yaml``'s
               ``out_dirs.exp2`` (RAJA_SRC/CAO_SRC below) -- since the 2026-09-14 folder
               rename this is ``publication_results/exp3_{raja,cao2018}/
               exp2_strategy_comparison_{raja,cao2018}_results.csv``, matching the
               manuscript's own Experiment 3 (Strategy Comparison) numbering even though
               the internal key/CSV filename still say "exp2" (historical, see the yaml's
               inline note). Written by ``exp3_a_strategy_comparison_{raja,cao2018}.py``.
  Filter     : ``selection == "all_channel"`` (full 32-channel montage gate) AND
               ``condition`` in {Proposed-Med, BLINKER-concat, MNE-annot} -- Proposed-Mean
               is not reported in this section and is excluded. One row per
               (session, condition), i.e. one PR operating point.
  Channels   : not electrode-specific -- ``all_channel`` is the best-channel-per-session
               oracle gate, not a fixed montage subset.
