...
def main() -> None:
    ...
    for i, cond in enumerate(P.CONDS):
        ...
        ax.bar(x + (i - 1.5) * width, values, width, label=cond,
               color=COLORS[cond], edgecolor=S.NAVY)
    ax.set_xticks(x)
    ax.set_xticklabels([label for _, label in METRICS])
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Macro-averaged score")
    S.style_axis(ax)
    legend = ax.legend(fontsize=S.FONT_CHROME, ncol=2)
    for text in legend.get_texts():
        text.set_color(S.NAVY)
    ax.set_title(f"Pooled performance over all {n_sessions} sessions", fontsize=S.FONT_CHROME)
    fig.tight_layout()
    ...
    ax.bar(x - width / 2, [P.macro(best, "raja", c)[2] for c in P.CONDS], width,
           label=P.DSN["raja"], color=S.DATASET_COLORS["Internal"], edgecolor=S.NAVY)
    ax.bar(x + width / 2, [P.macro(best, "cao", c)[2] for c in P.CONDS], width,
           label="Cao2018", color=S.DATASET_COLORS["Cao2018"], edgecolor=S.NAVY)
    ax.set_xticks(x)
    ax.set_xticklabels(P.CONDS, rotation=15)
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Macro-averaged $F_1$")
    S.style_axis(ax)
    legend = ax.legend(fontsize=S.FONT_CHROME)
    for text in legend.get_texts():
        text.set_color(S.NAVY)
    ax.set_title("Per-condition $F_1$ by dataset", fontsize=S.FONT_CHROME)
    fig.tight_layout()
