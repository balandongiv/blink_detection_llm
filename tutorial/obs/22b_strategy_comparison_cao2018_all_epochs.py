"""Strategy comparison — kleifges, nathanael_mne, dbo, dbo_drop across cao_2018 dataset (no epoch rejection).
...
def main() -> None:
    ...
    logger.info("Epoch health filtering: DISABLED (all epochs treated as healthy)")
    ...
    if USE_MULTITHREAD and pending_tasks:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    logger.info(
                        "done  session=%s  strategy=%s  n_epochs=%d  f1=%.4f",
                        session_name, strategy, result["n_epochs"], result["f1"],
                    )
        ...
        for name, fif, csv, strat in pending_tasks:
            ...
            try:
                ...
                logger.info(
                    "done     session=%s  strategy=%s  n_epochs=%d  f1=%.4f",
                    name, strat, result["n_epochs"], result["f1"],
                )
            except Exception as exc:
