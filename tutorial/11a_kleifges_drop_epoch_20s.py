"""Kleifges approach with epoch health-based dropping — 20-second epoch variant.

Same workflow as ``11a_kleifges_drop_epoch.py`` but with ``EPOCH_DURATION_S = 20.0``.
...
* ``*_process_algo`` — the 20-second algorithm epoch that contains each blink onset.
"""
...
ANNOTATION_CSV = OUT_DIR / "ear_eog_predicted_kleifges_20s.csv"
MASTERLIST_CSV = OUT_DIR / "blink_events_masterlist_kleifges_20s.csv"
REPORT_PATH = OUT_DIR / "blink_epoch_report_kleifges_20s.html"

# ---------------------------------------------------------------------------
# Parameters
# ---------------------------------------------------------------------------
EPOCH_DURATION_S: float = 20.0
...
def main() -> None:
    ...
    # Compute 20-s health values before attach_health_metadata so we can reuse them
