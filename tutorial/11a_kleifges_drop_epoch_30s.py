"""Kleifges approach with epoch health-based dropping — 30-second epoch variant.

Same workflow as ``11a_kleifges_drop_epoch_20s.py`` but with ``EPOCH_DURATION_S = 30.0``.
...
* ``*_process_algo`` — the 30-second algorithm epoch that contains each blink onset.
"""
...
ANNOTATION_CSV = OUT_DIR / "ear_eog_predicted_kleifges_30s.csv"
MASTERLIST_CSV = REPO_ROOT / "tests" / "blink_events_masterlist_kleifges_30s.csv"
REPORT_PATH = OUT_DIR / "blink_epoch_report_kleifges_30s.html"
