"""
Tutorial 19 – Strategy A **Step 1** batch evaluation across all subjects / segments.
...
2. For each pair prepare epoch data with prepare_epoch_detection_input, then run
   get_blink_position + map_concatenated_blinks_to_epochs per channel — mirroring
   Tutorial 11 but across all subjects.
3. Write per-pair outputs to
       experiment_output/<subject>/<segment>/strategy_a_step1_lane_summary.csv
       experiment_output/<subject>/<segment>/strategy_a_step1_best_lane_candidates.csv
       experiment_output/<subject>/<segment>/strategy_a_step1_metrics.json
...
# ---------------------------------------------------------------------------
# Settings (mirror Tutorial 11 / Tutorial 18)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Per-channel blink detection (Strategy A Step 1)
# ---------------------------------------------------------------------------
...
def run_get_blink_position_per_channel(
    ...
    """Call get_blink_position for each channel; return list of per-channel dicts."""
...
def process_pair(
    ...
    """Run Strategy A Step 1 on one pair and return a best-lane metrics dict."""
    result: dict = {
        "subject": subject,
        "segment": segment,
        "elapsed_s": float("nan"),
    ...
    try:
        ...
        # --- get_blink_position per channel ---
        ...
        if not summary.empty:
            ...
            if best_cr is not None and not best_cr["mapped_candidates"].empty:
                best_cr["mapped_candidates"].to_csv(
                    out_dir / "strategy_a_step1_best_lane_candidates.csv", index=False
                )

        # --- Persist per-pair metrics ---
        metrics_path = out_dir / "strategy_a_step1_metrics.json"
...
def main() -> None:
    print("=" * 70)
    print("Tutorial 19 – Strategy A Step 1 batch evaluation (all subjects)")
    print("=" * 70)
    ...
    results_csv = OUTPUT_ROOT / "strategy_a_step1_all_results.csv"
    ...
    if not successful.empty:
        ...
        agg_path = OUTPUT_ROOT / "strategy_a_step1_aggregate_summary.json"
