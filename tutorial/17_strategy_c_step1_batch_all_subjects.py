"""
Tutorial 17 – Strategy C **Step 1** batch evaluation across all subjects / segments.
...
2. For each pair run Strategy-C up to Stage 1 only (threshold learning +
   candidate scan) — intentionally stopping before FitBlinks / representative
   lane selection.
3. Write per-pair outputs to
       experiment_output/<subject>/<segment>/step1_lane_summary.csv
       experiment_output/<subject>/<segment>/step1_best_lane_candidates.csv
       experiment_output/<subject>/<segment>/step1_metrics.json
...
# ---------------------------------------------------------------------------
# Settings (mirror Tutorial 14 / 15)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Step 1 summary builder (same logic as Tutorial 14)
# ---------------------------------------------------------------------------
...
def process_pair(
    ...
    """Run Strategy C Step 1 on one pair and return a best-lane metrics dict."""
    result: dict = {
        "subject": subject,
        "segment": segment,
        "elapsed_s": float("nan"),
    ...
    try:
        ...
        if not summary.empty:
            ...
            # Save best-lane candidates
            best_channel = str(best["channel"])
            best_detection = next(
                (d for d in stage1.detections if d.channel == best_channel), None
            )
            if best_detection is not None and not best_detection.mapped_candidates.empty:
                best_detection.mapped_candidates.to_csv(
                    out_dir / "step1_best_lane_candidates.csv", index=False
                )

        # --- Persist per-pair metrics ---
        metrics_path = out_dir / "step1_metrics.json"
...
def main() -> None:
    print("=" * 70)
    print("Tutorial 17 – Strategy C Step 1 batch evaluation (all subjects)")
    print("=" * 70)
    ...
    results_csv = OUTPUT_ROOT / "step1_all_results.csv"
    ...
    if not successful.empty:
        ...
        agg_path = OUTPUT_ROOT / "step1_aggregate_summary.json"
